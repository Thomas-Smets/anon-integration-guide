...
// Rebalancer callback data
export function encodeRebalancerCallbackData(initiator: `0x${string}`, strategyHook: `0x${string}`, metaData: `0x${string}`): `0x${string}` {
    ...
    return encodeAbiParameters(
        ...
        [initiator, DEFAULT_MAX_CLAIM_FEE, DEFAULT_MAX_SWAP_FEE, DEFAULT_MAX_TOLERANCE, DEFAULT_MIN_LIQUIDITY_RATIO, strategyHook, emptyStrategyData, metaData],
    );
}

// Rebalancer metadata encoding
export function encodeRebalancerMetadata(params: {
    ...
    const strategyParams = encodeAbiParameters(
        ...
        [params.compoundLeftovers, params.optimalToken0Ratio, params.triggerLowerRatio, params.triggerUpperRatio, BigInt(params.minRebalanceTime), BigInt(params.maxRebalanceTime)],
    );

    return encodeOuterMetadata('default', strategyParams);
}

// Compounder callback data
export function encodeCompounderCallbackData(initiator: `0x${string}`): `0x${string}` {
    return encodeAbiParameters(
        ...
        [initiator, DEFAULT_MAX_CLAIM_FEE, DEFAULT_MAX_SWAP_FEE, DEFAULT_MAX_TOLERANCE, DEFAULT_MIN_LIQUIDITY_RATIO, '0x'],
    );
}

// Yield claimer callback data
export function encodeYieldClaimerCallbackData(initiator: `0x${string}`, feeRecipient: `0x${string}`): `0x${string}` {
    return encodeAbiParameters(
        [
            { name: 'initiator', type: 'address' },
...
// Merkl operator callback data
export function encodeMerklOperatorCallbackData(initiator: `0x${string}`, rewardRecipient: `0x${string}`): `0x${string}` {
    return encodeAbiParameters(
        [
            { name: 'initiator', type: 'address' },
...
// CowSwap token metadata
export function encodeCowSwapTokenMetadata(strategy: string, sellTokens: `0x${string}`[], buyToken: `0x${string}`): `0x${string}` {
...
// Coupled strategy metadata (empty inner data)
...
export function encodeCompounderCoupledCallbackData(initiator: `0x${string}`, strategy: string): `0x${string}` {
    ...
    return encodeAbiParameters(
        ...
        [initiator, DEFAULT_MAX_CLAIM_FEE, DEFAULT_MAX_SWAP_FEE, DEFAULT_MAX_TOLERANCE, DEFAULT_MIN_LIQUIDITY_RATIO, metaData],
    );
}

// Yield claimer callback data with coupled cowswap metadata
export function encodeYieldClaimerCoupledCallbackData(initiator: `0x${string}`, feeRecipient: `0x${string}`, strategy: string): `0x${string}` {
    const metaData = encodeCoupledStrategyMetadata(strategy);
    return encodeAbiParameters(
        [
            { name: 'initiator', type: 'address' },
